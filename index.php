...
class Dav {
    ...
    private function HTML(){
        ...
        .nav-input{background:transparent;border:1px solid transparent;color:var(--tx);font-size:15px;font-weight:500;width:400px;padding:6px 10px;border-radius:6px;transition:all 0.2s}
        ...
        th{text-align:left;padding:18px 28px;color:var(--pd);font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.8px;border-bottom:2px solid var(--bd);background:rgba(92,107,192,0.08);position:sticky;top:0;z-index:10}
        ...
        .ft{padding:20px;text-align:center;font-size:14px;color:#78909c;background:var(--hv);border-top:1px solid var(--bd);display:flex;flex-direction:row;justify-content:center;align-items:center;gap:8px;flex-shrink:0}
        ...
        .link-display input{width:100%;padding:8px 10px;border:2px solid var(--bd);border-radius:6px;background:rgba(255,255,255,0.8);color:var(--tx);font-size:13px;box-sizing:border-box}
        ...
        .chk{width:18px;height:18px;cursor:pointer;accent-color:var(--p)}
        .batch-bar{display:none;align-items:center;gap:12px;margin-left:auto;font-size:13px;background:var(--hv);padding:6px 12px;border-radius:8px;border:1px solid var(--bd)}
        ...
        <div class="bar">
            ...
            <?php endif;?>
            <form method="post" enctype="multipart/form-data" style="margin:0"><input type="hidden" name="t" value="<?=$csrf?>"><input type="hidden" name="cur_dir" value="<?=$this->req?>"><label class="btn bp" title="Supports files and folders (if browser allows)"><?=T('up')?><input type="file" name="f[]" hidden multiple onchange="this.form.submit()"></label></form>
            <form method="post" style="display:flex;gap:10px;margin:0;flex:1"><input type="hidden" name="t" value="<?=$csrf?>"><input type="hidden" name="cur_dir" value="<?=$this->req?>"><input name="md" placeholder="<?=T('new')?>" required style="padding:10px 14px;border:2px solid var(--bd);border-radius:10px;outline:none;background:rgba(255,255,255,0.8);color:var(--tx);flex:1;min-width:120px;max-width:280px"><button class="btn"><?=T('cr')?></button></form>
        ...
        <div class="main">
            <table>
                ...
                <tbody>
                    <?php if(count($l)<=2): ?><tr><td colspan="5" style="text-align:center;padding:80px 20px;color:#78909c;font-style:italic;font-size:16px"><?=T('emp')?></td></tr>
        ...
        function shareFile(n,t,expAt,maxUses,curUses,startAt){
            ...
            // Right Column: Exp, Delay, Max, Password
